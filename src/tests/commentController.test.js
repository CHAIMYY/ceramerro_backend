...
describe('Comment Controller', () => {
  ...
  describe('addComment', () => {
    ...
    it('should return 404 if blog post is not found', async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ message: 'Could not find the post' });
    });
    ...
    it('should return 500 if there is a server error', async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ message: 'Failed to add comment' });
  ...
  describe('deleteComment', () => {
    ...
    it('should return 403 if user is not authorized to delete the comment', async () => {
      ...
      mockComment.postedBy.toString = jest.fn().mockReturnValue(differentUserId.toString());
    ...
    it('should return 500 if there is a server error', async () => {
      ...
      expect(res.status).toHaveBeenCalledWith(500);
      expect(res.json).toHaveBeenCalledWith({ message: 'Failed to delete comment' });
      expect(console.error).toHaveBeenCalled();
    });
  });
  ...
  describe('getcomments', () => {
    ...
    it('should get all comments for a post', async () => {
      ...
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith({ success: true, comments: mockComments });
    });
