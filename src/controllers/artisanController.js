...
exports.updateProfile = async (req, res) => {
  try {
    ...
    const updatedFields = {
      ...
      socialMedia: {
        instagram:
          req.body.socialMedia?.instagram || user.socialMedia.instagram,
...
exports.getSellerStatistics = async (req, res) => {
  try {
    ...
    if (!artisan || artisan.role !== "artisan") {
      return res
        .status(404)
        .json({ message: "Artisan not found or invalid role" });
    }

    const totalArtisanProducts = await Product.countDocuments({
      userId: artisan._id,
    });
    const totalArtisanPosts = await Blog.countDocuments({
      creator: artisan._id,
    });
